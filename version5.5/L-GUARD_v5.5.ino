 ...
 */
 ...
 const unsigned long API_FORCE_SYNC_INTERVAL = 300000;    // 5min - force sync even if no change
 ...
 bool isSystemReallyStable() {
   ...
   bool noVibration = (!vibrationDetected);                         // No vibration
   bool notMoving = (currentMillis - lastMovementTime > SLEEP_IDLE_TIME);
   
   // All must be true
   if (accelStable && rollStable && speedStable && noVibration && notMoving) {
     stableCount++;
   } else {
 ...
 void loop() {
   ...
   // ========== API SYNC (CONTROLLED - NO SPAM!) ==========
   if (gprsConnected && !apiSyncInProgress) {
     bool timeTpSync = (currentMillis - lastApiSync >= API_MIN_INTERVAL);
     bool forceSync = (currentMillis - lastApiSent >= API_FORCE_SYNC_INTERVAL);
     bool hasChanges = hasSignificantChange();
     
     // Only sync if: time elapsed AND (has changes OR force sync OR accident flagged)
     if (timeTpSync && (forceSync || hasChanges || forceNextSync)) {
 ...
 void readADXL345() {
   ...
   if (adxl345Total > 1.5 || abs(currentSpeed) > 1.0) {
     lastMovementTime = millis();
   }
 }
 ...
 bool hasSignificantChange() {
   ...
   if (abs(currentBattPercent - prevSentBatteryPercent) >= 10) {
     anyChange = true;
   }
 ...
 bool httpPOST_Fixed(String jsonData) {
   ...
   if (downloadResp.indexOf("DOWNLOAD") >= 0) {
     ...
     if (dataResp.indexOf("OK") >= 0) {
       ...
       // Wait for response (max 30 seconds for cold starts)
       for (int i = 0; i < 15; i++) {
         delay(500);
         if (modemSerial.available()) {
           ...
           if (result.indexOf("+HTTPACTION") >= 0) {
             if (result.indexOf(",200,") >= 0 || result.indexOf(",201,") >= 0) {
               ...
               Serial.println("   ❌ API Error: " + result);
             }
             break;
           }
         }
