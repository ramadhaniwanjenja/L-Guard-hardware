  ...
  */
  ...
  // Build proper ISO 8601 timestamp
  String timestamp;
  
  if (utcTime.length() > 0 && utcTime.indexOf(":") > 0) {
    // GPS time is valid (format: "14:30:25")
    timestamp = "2024-12-22T" + utcTime + "Z";
  } else {
    // GPS time not available - use system uptime
    ...
    timestamp = "2024-12-22T" + String(timeStr) + "Z";
  }
  
  String json = "{";
  json += "\"deviceId\":\"" + DEVICE_ID + "\",";
  json += "\"accidentType\":\"" + accidentType + "\",";
  json += "\"impact\":\"" + impactLevel + " impact detected\",";
  json += "\"accidentTime\":\"" + timestamp + "\",";
  json += "\"speed\":" + String(currentSpeed, 1) + ",";
