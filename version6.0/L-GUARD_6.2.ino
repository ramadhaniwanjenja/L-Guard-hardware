/*
 * ╔══════════════════════════════════════════════════════════════╗
 * ║   L-GUARD VEHICLE SAFETY SYSTEM  v6.2                       ║
 * ║   ADXL375 (±200g) + MPU6050 + GPS + GSM                     ║
 * ╠══════════════════════════════════════════════════════════════╣
 * ║  ✅ 800 Hz free-running ADXL375 — no missed spikes           ║
 * ║  ✅ modemDelay() — ADXL keeps sampling during modem waits    ║
 * ║  ✅ Per-axis peak capture (peakAxisX/Y/Z)                    ║
 * ║  ✅ Accident detection uses peak, not live reading            ║
 * ║  ✅ Battery guard — N/A if ADC floating                      ║
 * ║  ✅ GPRS auto-reconnect after 3 consecutive API fails        ║
 * ║  ✅ GPRS auto-reconnect if disconnected in loop              ║
 * ║  ✅ OTA update via Device network                   ║
 * ║  ✅ WiFi turns OFF after OTA setup — SIM takes over          ║
 * ║  ✅ Priority sync on band-change                              ║
 * ║  ✅ Real-time sync every 3s                                   ║
 * ║  ✅ 60s peak decay                                            ║
 * ║  ✅ Rollover fix (tilt + impact gate)                         ║
 * ║  ✅ accidentType/impactLevel reset every check cycle          ║
 * ╠══════════════════════════════════════════════════════════════╣
 * ║  THRESHOLDS TUNED FROM REAL BIKE TEST (2026-06-30)           ║
 * ║  ── ADXL375 Impact (road bumps peaked at 18g single-axis) ── ║
 * ║     LOW     : 25g   (was  8g)                                ║
 * ║     MEDIUM  : 40g   (was 25g)                                ║
 * ║     HIGH    : 75g   (was 50g)                                ║
 * ║     EXTREME : 150g  (was 100g)                               ║
 * ║  ── MPU6050 Tilt (normal riding lean: -37° to +22°) ──────── ║
 * ║     TILT    : 60°   (was 45°)                                ║
 * ║     ROLLOVER: 100°  (was 70°)                                ║
 * ║  ── Speed (max observed: 48.7 km/h) ──────────────────────── ║
 * ║     LIMIT   : 50 km/h  (was 40 km/h)                        ║
 * ║     MIN     :  2 km/h  (was  5 km/h — GPS noise filter)     ║
 * ║  ── Change Detection ──────────────────────────────────────── ║
 * ║     ANGLE   : 20°   (was 30°)                                ║
 * ║  ── Speed Drop (accident condition) ───────────────────────── ║
 * ║     previousSpeed > 15 km/h  (was 40 km/h)                  ║
 * ║     currentSpeed  <  5 km/h  (was 10 km/h)                  ║
 * ║     requires impact > LOW threshold to confirm crash          ║
 * ╚══════════════════════════════════════════════════════════════╝
 *
 *  OTA INSTRUCTIONS:
 *  1. Power on device — it creates its own hotspot "L-Guard-OTA"
 *  2. Connect PC to "L-Guard-OTA" WiFi, password: lguard2024
 *  3. PlatformIO → Ctrl+Alt+U
 *     OR Arduino IDE → Tools → Port → 192.168.4.1
 *  4. Upload — enter OTA password: lguard2024
 *  5. After upload device reboots, WiFi turns off, SIM takes over
 *
 *  WIRING:
 *    ADXL375  SDA→GPIO21  SCL→GPIO22  CS→3.3V  SDO→GND (0x53)
 *    MPU6050  SDA→GPIO21  SCL→GPIO22  AD0→GND  (0x69)
 *    Cancel button  GPIO27→GND
 */
...
// ── OTA CONFIG ────────────────────────────────────────────────
const char* OTA_AP_SSID  = "L-Guard-OTA";  // hotspot ESP32 creates
const char* OTA_HOSTNAME = "lguard-vehicle001";
const char* OTA_SECRET   = "lguard2024";
...
#define OK_BUTTON_PIN       27 // was 26
#define MENU_BUTTON_PIN     26 // was 27
...
String EMERGENCY_CONTACT     = "+250780904149";

// ── SPEED CONFIG ───────────────────────────────────────────────
const float SPEED_LIMIT              = 50.0;  // was 40 — max observed was 48.7 km/h on bike
const int   MIN_SATELLITES_FOR_SPEED = 4;
const float MIN_SPEED_THRESHOLD      = 2.0;   // was 5.0 — GPS at stop showed 1.5-1.9 km/h noise, 2.0 filters it
const float MIN_GPS_HDOP             = 3.0;

// ── ADXL375 THRESHOLDS ────────────────────────────────────────
const float ADXL375_LOW_THRESHOLD     =  25.0;  // was 15 — single axis hits 18g on bumps (normal impact on bike), 25g is safe
const float ADXL375_MEDIUM_THRESHOLD  =  40.0;  // was 25 — give buffer above LOW, real collision
const float ADXL375_HIGH_THRESHOLD    =  75.0;  // was 50 — severe crash
const float ADXL375_EXTREME_THRESHOLD = 150.0;  // was 100 — catastrophic, sensor goes to 200g max
const float MPS2_TO_G                 = 1.0 / 9.80665;

// ── MPU6050 / GYRO THRESHOLDS ──────────────────────────────────
const float MPU6050_TILT_THRESHOLD     = 60.0;   // was 45
const float MPU6050_ROLLOVER_THRESHOLD = 100.0;  // was 70
const float GYRO_ROTATION_THRESHOLD   =  3.0;
const float GYRO_FAST_THRESHOLD       =  6.0; 
const int   ROLLOVER_STABILITY_COUNT  =  3;
const float ROLLOVER_MIN_IMPACT       =  5.0;
...
const unsigned long API_FORCE_SYNC_INTERVAL = 900000UL;
...
const unsigned long REALTIME_INTERVAL = 3000UL; // send every 3s
...
const float ANGLE_CHANGE_THRESHOLD    = 20.0; // was 30.0
...
const int   BATTERY_ADC_MIN_RAW = 100;

// ── GPRS RECONNECT ────────────────────────────────────────────
const int MAX_FAILS_BEFORE_RECONNECT = 3;
int consecutiveApiFails = 0;
...
// ── ADXL375 LIVE READINGS ─────────────────────────────────────
float adxl375X = 0, adxl375Y = 0, adxl375Z = 0, adxl375Total = 0;

// ── ADXL375 PEAK READINGS ─────────────────────────────────────
float adxl375MaxImpact = 0;
float peakAxisX        = 0;
float peakAxisY        = 0;
float peakAxisZ        = 0;
...
// ── SAMPLE COUNTER ────────────────────────────────────────────
unsigned long adxlSampleCount = 0;
...
void       setupModem();
bool       connectGPRS();
void       checkAndReconnectGPRS();
void       modemDelay(unsigned long ms);
void       readADXL375();
void       readMPU6050();
void       updatePeaks();
void       decayPeaks();
...
// ══════════════════════════════════════════════════════════════
//  OTA SETUP
//  Tries WiFi for 15s at boot only.
//  If hotspot found → OTA enabled.
//  If not found → continues normally, WiFi stays off.
// ══════════════════════════════════════════════════════════════
void handleTelnet() {
  ...
  if (telnetClient && telnetClient.connected()) {
    while (Serial.available()) telnetClient.write(Serial.read());
  }
}

void setupOTA() {
  Serial.println("OTA: starting L-Guard hotspot...");

  // ESP32 becomes the hotspot — no phone needed
  WiFi.mode(WIFI_AP);
  WiFi.softAP(OTA_AP_SSID, OTA_SECRET);

  IPAddress ip = WiFi.softAPIP();
  Serial.println("✅ Hotspot started!");
  Serial.println("   SSID    : L-Guard-OTA");
  Serial.println("   Password: lguard2024");
  ...
  ArduinoOTA.onStart([]() {
    Serial.println("\n🔄 OTA Update starting — do not power off!");
    apiSyncInProgress = true;
  });
  ArduinoOTA.onEnd([]() {
    Serial.println("\n✅ OTA complete — rebooting now");
  });
  ...
  Serial.println("✅ OTA ready — connect PC to L-Guard-OTA then upload");
}
...
String impactBandName(ImpactBand b) {
  switch(b) {
    case IB_EXTREME: return "EXTREME>150g";
    case IB_HIGH:    return "HIGH>75g";
    case IB_MEDIUM:  return "MEDIUM>40g";
    case IB_LOW:     return "LOW>25g";
    default:         return "NONE";
  }
}
String tiltBandName(TiltBand b) {
  switch(b) {
    case TB_ROLLOVER: return "ROLLOVER>100deg";
    case TB_WARN:     return "WARN>60deg";
    default:          return "FLAT";
  }
}
String speedBandName(SpeedBand b) {
  switch(b) {
    case SB_OVERSPEED: return "OVER>50kmh";
    case SB_FAST:      return "FAST20-50";
    case SB_NORMAL:    return "NORMAL2-20";
...
// ══════════════════════════════════════════════════════════════
//  modemDelay — keeps ADXL375 sampling during modem blocking
// ══════════════════════════════════════════════════════════════
void modemDelay(unsigned long ms) {
...
// ══════════════════════════════════════════════════════════════
void checkAndReconnectGPRS() {
  if (consecutiveApiFails < MAX_FAILS_BEFORE_RECONNECT) return;

  Serial.println("\n⚠️  " + String(consecutiveApiFails) +
                 " consecutive fails — reconnecting GPRS...");
  ...
  Serial.print("  . Reactivating... ");
  if (sendATCommand("AT+CGACT=1,1", 15000).indexOf("OK") >= 0) {
    modemDelay(1000);
    if (sendATCommand("AT+CGPADDR=1", 3000).indexOf("+CGPADDR") >= 0) {
      gprsConnected = true;
      Serial.println("✅ GPRS reconnected!\n");
      return;
    }
  }
  Serial.println("❌ GPRS reconnect failed — will retry\n");
}
...
// ══════════════════════════════════════════════════════════════
void checkRangeChange() {
  ImpactBand newImpact = getImpactBand(adxl375MaxImpact);
  ...
  if (newImpact != currentImpactBand) {
    Serial.printf("\n⚡ IMPACT BAND: [%s] → [%s]  (peak %.2fg  live %.2fg)\n",
...
// ══════════════════════════════════════════════════════════════
void updatePeaks() {
  if (adxl375MaxImpact > peakImpactG)  peakImpactG  = adxl375MaxImpact;
...
void setup() {
  ...
  delay(2000);
  Serial.println("\n\n╔══════════════════════════════════════════════════════╗");
  Serial.println("║   L-GUARD v6.2 — With OTA Update Support            ║");
  Serial.println("╚══════════════════════════════════════════════════════╝\n");
  ...
  // ── OTA first — before sensors and modem ──────────────────
  setupOTA();
  ...
  prevSentBatteryPercent = -1;
  lastApiSent = millis();

  Serial.println("\n╔══════════════════════════════════════════════════════╗");
  Serial.println("║  ADXL375 SAMPLE RATE : 800 Hz (free-running)         ║");
  Serial.println("║  MODEM DELAY         : NON-BLOCKING (peak safe)      ║");
  Serial.println("║  GPRS RECONNECT      : AUTO after 3 fails            ║");
  Serial.println("║  BATTERY             : Guard against floating ADC    ║");
  Serial.println("║  OTA UPDATE          : Own hotspot L-Guard-OTA       ║");
  Serial.println("║  ACCIDENT DETECTION  : ACTIVE  (peak-based)          ║");
  ...
  Serial.println("║  SPEED ALERT         : > 50 km/h                     ║");
...
float readBatteryVoltage() {
  ...
  int avgRaw = total / 5;
  if (avgRaw < BATTERY_ADC_MIN_RAW) return -1.0;
  return (avgRaw / ADC_MAX_VALUE) * ADC_REF_VOLTAGE * BATTERY_DIVIDER;
}
...
void checkAccident() {
  ...
  accidentType = "";  // added to clean then accident log type, and impact level
  impactLevel  = "";
  ...
  bool extremeImpact = (adxl375MaxImpact > ADXL375_EXTREME_THRESHOLD);
  bool highImpact    = (adxl375MaxImpact > ADXL375_HIGH_THRESHOLD);
  bool mediumImpact  = (adxl375MaxImpact > ADXL375_MEDIUM_THRESHOLD);

  bool rolloverSustained = mpuAvailable
    ? (rolloverCount >= ROLLOVER_STABILITY_COUNT && adxl375MaxImpact > ROLLOVER_MIN_IMPACT)
    : false;
...
bool speedDrop = (gpsFixed && previousSpeed > 15.0 && currentSpeed < 5.0
                  && (previousSpeed - currentSpeed) >= SPEED_DROP_THRESHOLD
                  && adxl375MaxImpact > ADXL375_LOW_THRESHOLD);
  ...
  if (accidentCondition) {
    ...
    Serial.printf( "  Peak Impact  : %.2fg  ← 800Hz free-running peak\n", adxl375MaxImpact);
    Serial.printf( "  Peak Axis X  : %.2fg\n",   peakAxisX);
    Serial.printf( "  Peak Axis Y  : %.2fg\n",   peakAxisY);
    Serial.printf( "  Peak Axis Z  : %.2fg\n",   peakAxisZ);
...
bool hasSignificantChange() {
  ...
  float accelDiff = abs(adxl375MaxImpact - prevSentAccelTotal);
  ...
  bool anyChange =
    ...
    (curBatt >= 0 && abs(curBatt - prevSentBatteryPercent) >= 20) ||
...
// ══════════════════════════════════════════════════════════════
String buildTelemetryJSON() {
  int battPct = batteryPercentFromVoltage(readBatteryVoltage());
  String battStr = (battPct < 0) ? "null" : String(battPct);
  ...
  json += "\"accelerationX\":" + String(peakAxisX, 2)        + ",";
  json += "\"accelerationY\":" + String(peakAxisY, 2)        + ",";
  json += "\"accelerationZ\":" + String(peakAxisZ, 2)        + ",";
  ...
  json += "\"engineTemp\":0,";
  json += "\"fuelLevel\":0,";
  json += "\"batteryLevel\":"  + battStr                     + ",";
  json += "\"signalStrength\":85,";

  json += "\"rawData\":{";
  json += "\"totalAccel\":"    + String(adxl375MaxImpact, 2) + ",";
...
bool httpPOST_Fixed(String jsonData) {
  ...
  modemDelay(1000); readModemResponse(2000);
      ...
      for (int i = 0; i < 15; i++) {
        ...
        if (modemSerial.available()) {
          ...
          if (r.indexOf("+HTTPACTION") >= 0) {
            ...
            Serial.println(success ? "   HTTP 200 OK" : "   HTTP Error: " + r);
            break;
          }
        }
      }
...
void sendTrackingDataToAPI() {
  ...
  Serial.println("\n📤 Telemetry → [" + (syncReason.length() > 0 ? syncReason : "PERIODIC") + "]"
                 + "  peak:" + String(adxl375MaxImpact, 2) + "g");
  ...
  if (httpPOST_Fixed(buildTelemetryJSON())) {
    ...
    prevSentLon            = currentLon;
    prevSentSpeed          = currentSpeed;
    prevSentAccelTotal     = adxl375MaxImpact;
    prevSentRollAngle      = rollAngle;
    ...
    consecutiveApiFails++;
    Serial.println("   ❌ Telemetry failed (streak: " + String(consecutiveApiFails) + ")");
  }
}
...
void printCompactStatus() {
  ...
  Serial.print("g Pk:");
  Serial.print(adxl375MaxImpact, 1);
  Serial.print("g(X:");
  Serial.print(peakAxisX, 1);
  Serial.print(" Y:");
  Serial.print(peakAxisY, 1);
  Serial.print(" Z:");
  Serial.print(peakAxisZ, 1);
  Serial.print(")");
  Serial.print(" │ Tilt:");
  ...
  Serial.print(hdop, 1);
  Serial.print(" │ Bat:");
  if (battPct < 0) Serial.print("N/A");
  else { Serial.print(battPct); Serial.print("%"); }
  Serial.print(" │ GSM:");
  Serial.print(modemReady ? "OK" : "NO");
  Serial.print(" │ API:");
  Serial.print(gprsConnected ? "OK" : "NO");
  Serial.print(" │ OTA:");
  Serial.print(otaAvailable ? "ON" : "OFF");
  Serial.print(" │ Last:");
  Serial.print((millis() - lastApiSent) / 1000);
  Serial.print("s │ Fails:");
  Serial.print(consecutiveApiFails);
  Serial.print(" │ Smpls:");
  Serial.print(adxlSampleCount);
  Serial.println(" │");
  adxlSampleCount = 0;
}
...
void loop() {
 ...
 handleTelnet();
  ...
  //  1. CANCEL BUTTON — must be first, before any blocking calls
  // This way pressing the button is always caught immediately
  if (accidentDetected && !smsSent) {
    handleCancelButton();
    if (!accidentDetected) return; // cancelled — skip everything else this cycle
  }

  // 2. ADXL375 — free-running every iteration
  readADXL375();

  // 3. MPU6050 + band check + accident — every 50ms
  ...
  // 6. API sync — skip entirely if accident is pending cancel
  if (!apiSyncInProgress && !accidentDetected) {
    if (!gprsConnected) {
      Serial.println("📡 GPRS lost — attempting reconnect...");
      connectGPRS();
    } else {
      checkAndReconnectGPRS();
    }
    bool forceSync    = (now - lastApiSent >= API_FORCE_SYNC_INTERVAL);
    bool realtimeSync = (now - lastApiSent >= REALTIME_INTERVAL);
    bool hasChanges   = hasSignificantChange();
    if (immediateSync || hasChanges || forceSync || forceNextSync || realtimeSync) {
  ...
  // 10. Light sleep
  if (!otaAvailable && isSystemReallyStable() &&
      !accidentDetected && !isSpeeding && !apiSyncInProgress) {
